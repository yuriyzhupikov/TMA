...
│
├─ libs
│  ├─ kernel/                           # базовые типы, Result, error-коды
│  │  ├─ index.ts                       # реэкспорт основных утилит
│  │  ├─ result.ts                      # Result<T, E>, Ok/Err helper'ы
│  │  ├─ errors.ts                      # базовые классы ошибок домена/инфры
│  │  └─ types.ts                       # общие utility-типы
│  │
│  ├─ config/                           # общая конфигурация приложения
│  │  ├─ index.ts                       # публичный экспорт конфига
│  │  ├─ config.ts                      # функции загрузки/мержа конфигов из env
│  │  └─ config.schema.ts               # схема валидации env (zod/joi)
│  │
│  ├─ postgres/                         # модуль работы с Postgres
│  │  ├─ index.ts                       # экспорт postgres.module и сущностей
│  │  ├─ postgres.module.ts             # Nest-модуль PG, подключение TypeORM/Prisma/knex
│  │  ├─ entities/                      # сущности БД
│  │  │  ├─ user.entity.ts              # таблица пользователей SaaS
│  │  │  ├─ company.entity.ts           # таблица компаний/тенантов
│  │  │  ├─ project.entity.ts           # таблица проектов TMA
│  │  │  ├─ project-config.entity.ts    # таблица конфигов проектов
│  │  │  ├─ player.entity.ts            # таблица игроков (в рамках проекта)
│  │  │  └─ player-progress.entity.ts   # прогресс игроков (баланс, уровень, даты)
│  │  └─ migrations/                    # SQL-миграции для схемы БД
│  │     ├─ 001-init.sql                # начальное создание схемы и таблиц
│  │     └─ 002-add-player-progress.sql # добавление таблицы/полей прогресса
│  │
│  ├─ redis/                            # модуль работы с Redis
│  │  ├─ index.ts                       # экспорт RedisModule и сервисов
│  │  ├─ redis.module.ts                # Nest-модуль Redis-подключения
│  │  ├─ redis.service.ts               # общий сервис доступа к Redis (кэш и т.п.)
│  │  └─ redis-rate-limit.service.ts    # сервис rate limit/антиспама
│  │
│  ├─ kafka/                            # модуль работы с Kafka
│  │  ├─ index.ts                       # экспорт KafkaModule и утилит
│  │  ├─ kafka.module.ts                # Nest-модуль Kafka-продьюсеров/консьюмеров
│  │  ├─ kafka.config.ts                # конфигурация подключения Kafka
│  │  ├─ kafka-producer.ts              # обёртка продьюсеров (отправка событий)
│  │  ├─ kafka-consumer.ts              # базовая реализация консьюмеров
│  │  └─ events/                        # определения типов событий
│  │     ├─ analytics-events.ts         # типы событий аналитики (click, reward и т.п.)
│  │     └─ player-events.ts            # типы событий по игрокам
│  │
│  ├─ telegram/                         # общие утилиты для Telegram (backend-часть)
│  │  ├─ index.ts                       # экспорт telegram утилит
│  │  ├─ telegram-initdata.ts           # разбор и валидация initData WebApp
│  │  ├─ telegram-bot.client.ts         # HTTP-клиент к Telegram Bot API
│  │  └─ types.ts                       # TS-типы для Telegram Update, WebApp и т.п.
│  │
│  ├─ auth/                             # общая auth-инфраструктура
│  │  ├─ index.ts                       # экспорт стратегии и guard'ов
│  │  ├─ jwt.strategy.ts                # JWT-стратегия для Nest (passport)
│  │  ├─ auth.guard.ts                  # guard, проверяющий токен/авторизацию
│  │  └─ current-user.decorator.ts      # декоратор получения текущего пользователя
│  │
│  ├─ dto/                              # общие DTO-контракты admin/runtime
│  │  ├─ index.ts                       # сводный экспорт DTO
│  │  │
│  │  ├─ admin/                         # DTO для /admin/*
│  │  │  ├─ auth.dto.ts                 # LoginDto, MeDto, RefreshTokenDto
│  │  │  ├─ tenants.dto.ts              # Create/UpdateTenantDto, TenantListQueryDto
│  │  │  ├─ users.dto.ts                # Create/UpdateUserDto, UserListQueryDto
│  │  │  ├─ projects.dto.ts             # Create/UpdateProjectDto, ProjectListQueryDto
│  │  │  ├─ configs.dto.ts              # UpsertConfigDto, PublishConfigDto, ConfigListQueryDto
│  │  │  └─ analytics.dto.ts            # AnalyticsQueryDto, ProjectAnalyticsDto и др.
│  │  │
│  │  ├─ runtime/                       # DTO для /runtime/*
│  │  │  ├─ runtime-init.dto.ts         # запрос init со стороны TMA
│  │  │  ├─ runtime-init-response.dto.ts# ответ init (игрок, баланс, конфиг)
│  │  │  ├─ runtime-event.dto.ts        # запрос события (тип, payload)
│  │  │  └─ runtime-event-response.dto.ts # ответ события (новое состояние)
│  │  │
│  │  └─ common/                        # общие DTO-утилиты
│  │     ├─ pagination.dto.ts           # DTO для пагинации (page, limit), обёртка ответа
│  │     ├─ id.dto.ts                   # DTO для параметров id/tenantId/projectId
│  │     ├─ date-range.dto.ts           # DTO диапазона дат (from, to)
│  │     └─ filters.dto.ts              # DTO базовых фильтров (search, status, sort)
│  │
│  ├─ logging/                          # единый логгер/трейсинг
│  │  ├─ index.ts                       # экспорт логгера/трейсинга
│  │  ├─ logger.module.ts               # Nest-модуль логгера
│  │  └─ tracing.ts                     # обвязка для трассировки (open-telemetry, id запросов)
│  │
│  └─ prometheus/                       # общий модуль интеграции с Prometheus
│     ├─ index.ts                       # экспорт PrometheusModule/Service
│     ├─ prometheus.service.ts         # регистрация и обновление метрик
│     └─ prometheus.module.ts          # Nest-модуль для подключения в сервисы
│
├─ infra
│  ├─ docker/
│  │  ├─ backend-core.Dockerfile       # образ backend-core
│  │  ├─ worker-jobs.Dockerfile        # образ воркера
│  │  ├─ admin-web.Dockerfile          # образ админки
│  │  └─ tma-frontend.Dockerfile       # образ TMA-фронта
│  │
│  ├─ k8s/
│  │  ├─ backend-core.yaml             # деплой/сервис backend-core в Kubernetes
│  │  ├─ worker-jobs.yaml              # деплой/сервис воркера в Kubernetes
│  │  ├─ ingress.yaml                  # ingress: /admin/* и /runtime/* → backend-core
│  │  ├─ postgres/                     # манифесты Postgres (statefulset/service/secret)
│  │  ├─ redis/                        # манифесты Redis
│  │  ├─ kafka/                        # манифесты Kafka (single/broker/cluster)
│  │  └─ clickhouse/                   # манифесты ClickHouse
│  │
│  └─ nginx/
│     └─ nginx.conf                    # proxy-конфиг для docker-compose стенда
│
└─ docs
   ├─ architecture/
